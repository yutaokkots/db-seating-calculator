        ...
        <>
            <div className="relative w-full h-full">    
                {top == true && frontBackDiff > 0 &&
                    <>
                        <div className={`absolute ${top ?' top-0' : 'bottom-0'} ${left ? 'right-0' : 'left-0'}  bg-red-50  ${levelFB > 0 ? 'w-[30px]' : ""}    h-[2px]     rounded-md`}>
                            <div className={`${ frontBackDiff > 0 ? "" : ""} `}></div>
                        </div>
                        <div className={`absolute ${top ?' top-2' : 'bottom-2'} ${left ? 'right-0' : 'left-0'}  bg-red-50  ${levelFB > 0 ? 'w-[10px]' : ""}    h-[2px]     rounded-md`}>
                            <div className={`${ frontBackDiff > 0 ? "" : ""} `}></div>
                        </div>
                    </>
                }
                ...
                {left == true && leftRightDiff > 0 &&
                    <>
                        <div className={`absolute ${top ?' bottom-0' : 'top-0'} ${left ? 'left-0' : 'right-0'}  bg-red-50 ${levelLR > 0 ? 'h-[30px]' : ""} rounded w-[2px]`}>
                            <div className={``}></div>
                        </div>
                        <div className={`absolute ${top ?' bottom-0' : 'top-0'} ${left ? 'left-2' : 'right-2'}  bg-red-50 ${levelLR > 0 ? 'h-[10px]' : ""} rounded w-[2px]`}>
                            <div className={``}></div>
                        </div>
                    </>
                }
